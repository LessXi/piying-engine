...
export class WaveScreen {
  ...
  update(dt, light, camera) {
    ...
    if (light) {
      ...
      // ★ 阴影偏移必须**随场景尺度缩放**，不能写死常量。
      //   阴影贴图把 [near, far] 压进 0..1，所以"1 个归一化单位"对应的真实距离
      //   是 (far - near) 米。若 bias 写死成 0.0042，在 range≈3.7m 的场景里就等于
      //   容忍 1.5cm 的深度差 —— 而把皮影贴在幕布正前方 2cm 的场景，它的深度差
      //   刚好落进这个容差里，于是**整个人影被判成"受光"而消失**，只在某些部位
      //   露出碎片。（原项目里人物离幕布 25cm，侥幸没暴露。）
      //   这里按物理量反算：离幕布 3mm 以内算"贴着幕布"、视为受光。
      const biasMeters = 0.003;
      u.uShadowBias.value = biasMeters / Math.max(far - near, 1e-3);
