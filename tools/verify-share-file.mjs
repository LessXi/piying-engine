...
const FILE = process.argv[2] || 'yingchuang-shadow-play.html';
const baseName = FILE.split(/[\\/]/).pop();
const isolated = join(mkdtempSync(join(tmpdir(), 'share-iso-')), baseName);
writeFileSync(isolated, readFileSync(resolve(FILE)));
console.log('隔离副本:', isolated);
console.log('该目录内容:', readdirSync(join(isolated, '..')).join(', ') || '(空)');
